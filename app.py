...
if 'map_data' not in st.session_state:
    st.session_state.map_data = pd.DataFrame({
        ...
        'Mecha_Armies': [20, 20, 20, 20, 20, 20],  # تم إصلاح القيم هنا
        'Plasma_Cores': [100, 150, 120, 200, 180, 250] # تم إصلاح القيم هنا
    })
...
if st.sidebar.button("📡 تفعيل الاتصال والسيطرة"):
    if commander_name:
        ...
        df.loc[df['Sector'] == starting_sector, 'Mecha_Armies'] = 40 
...
if attacker:
    ...
    if my_sectors:
        ...
        current_garrison = int(df[df['Sector'] == from_sector]['Mecha_Armies'].iloc[0])
        ...
        if max_attackers >= 1:
            ...
            if st.sidebar.button("🚀 إطلاق جحافل الروبوتات (Launch Attack)"):
                defender_force = int(df[df['Sector'] == target_sector]['Mecha_Armies'].iloc[0])
                defender_name = str(df[df['Sector'] == target_sector]['Overlord'].iloc[0])
                ...
                if battle_result > 0:
                    ...
                    st.session_state.map_data = df
                    st.balloons()
                    st.success(f"💥 تم اختراق النظم! القائد {attacker} يسيطر على {target_sector}!")
                    st.rerun()
                else:
                    df.loc[df['Sector'] == target_sector, 'Mecha_Armies'] = abs(battle_result)
                    st.session_state.map_data = df
                    st.error(f"💀 فشل الهجوم! صمدت دفاعات {target_sector}. المتبقي للعدو: {abs(battle_result)} ميكا.")
                    st.rerun()
        else:
            st.sidebar.warning("لا توجد ميكا كافية للهجوم (اترك روبوت واحد للدفاع).")
    else:
        st.sidebar.warning("يجب أن تسيطر على قطاع طاقة واحد على الأقل للبدء.")

# --- عرض الخريطة الرقمية والبيانات ---
